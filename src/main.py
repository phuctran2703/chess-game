...
from src.ai.visualizer import AIVisualizerGUI
from src.ai.ai_vs_ai import AIvsAIGUI
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == '--ai-visualizer':
            ...
            app = AIvsAIGUI(root)
            root.title("Chess Game - AI vs AI")
        else:
...
def show_mode_selection(root):
    ...
    ai_viz_button = tk.Button(
        selection_frame, 
        text="Người vs AI (hiển thị quá trình suy nghĩ của AI)", 
        font=("Arial", 14),
        bg="#2196F3", fg="white",
        height=2,
    ...
    ai_vs_ai_button = tk.Button(
        selection_frame, 
        text="AI vs AI (AI tự chơi với nhau)", 
        font=("Arial", 14),
        bg="#9C27B0", fg="white",
        height=2,
